...
setInterval(function() {
	console.log('reset global starting in 1 second...');
    fabricPainter.brush_globals.drawPerSecond = 0;
}, 1000);
...
fabricPainter.addSquare = function(data, top, left, angle, height, width) {
	canvas.add(new fabric.Rect({
		top: top || data.layerY || Math.random()*canvas.width,
		left: left || data.layerX || Math.random()*canvas.width,
...
fabricPainter.addLine = function(data, angle, height, top, left, width) {
	canvas.add(new fabric.Rect({
		...
		left: left || data.layerX || 100,
		top: top || data.layerY || 100,
		angle: angle || fabricPainter.brush_globals.angle,
...
fabricPainter.drawTree = function(data, max_iterations){
	...
	for(var i = 0; i <= max_iterations; i++) {
		fabricPainter.addCircle(data.layerY + (i * i), data.layerX, i);
	}
	return;
};
...
fabricPainter.drawInvertedTree = function(data, max_iterations){
	...
	for(var i = 0; i <= max_iterations; i++) {
		fabricPainter.addCircle(data.layerY + (i * (i + i)), data.layerX, max_iterations - i);
	}
	return;	
};

fabricPainter.drawCopyCat = function(data) {
	fabricPainter.addCircle(data.layerY, data.layerX, fabricPainter.brush_globals.size);

	// draw mirrored but smaller copies of original
	var size = fabricPainter.brush_globals.size + 40;
	fabricPainter.addCircle(data.layerY - size, data.layerX - size, fabricPainter.brush_globals.size/3);
	fabricPainter.addCircle(data.layerY + size, data.layerX + size, fabricPainter.brush_globals.size/3);
	return;
};
...
fabricPainter.drawEerieFollower = function(data, timeout) {
	...
	fabricPainter.addCircle(data.layerY, data.layerX, fabricPainter.brush_globals.size);
	setTimeout(function(){
		fabricPainter.addCircle(data.layerY+100, data.layerX, fabricPainter.brush_globals.size/3);
...
fabricPainter.drawGraffiti = function(data) {
	...
	fabricPainter.brush_globals.drawPerSecond += 1;
	//fabricPainter.brush_globals.drawRandCount += 1;
	console.log('drawing per second global: '+fabricPainter.brush_globals.drawPerSecond);

	if(Math.random() * 15 < 5) {

		// randomize placement of second "drip"
		fabricPainter.addLine(data, 0, bar_height, data.layerY + (bar_height / 2), data.layerX, Math.random() * max_bar_width);
	}
	return;
};
...
fabricPainter.addRandomRect = function(data, angle) {
	fabricPainter.addSquare(data, data.layerY, data.layerX, angle, Math.random() * fabricPainter.brush_globals.size, Math.random() * fabricPainter.brush_globals.size);
	return;
};

fabricPainter.drawBubblesSimple = function(data) {
	for(var i = 0; i <= 2; i++) {
		fabricPainter.addCircle(data.layerY + Math.random(), data.layerX + Math.random(), fabricPainter.brush_globals.size);
	}
	return;
};
...
fabricPainter.drawEchoes = function(data) {
	...
	for(var i = 0; i <= 5; i++) {
		fabricPainter.addCircle(data.layerY + (i * i * i), data.layerX + (i * i * i), fabricPainter.brush_globals.size);
	}
	return;
};
...
fabricPainter.drawDiamondCross = function(data) {
	...
	fabricPainter.addSquare(data, data.layerY, Math.random() * f, 45, f/4, f/4);
	fabricPainter.addSquare(data, Math.random() * f, data.layerX, 45, f/4, f/4);
	return;
};
